...
from langchain_components.runtime.events import STEP_COMPLETED, STEP_FAILED, STEP_STARTED, RuntimeEvent
...
class RuntimeExecutor:
    """Executes exactly ONE RuntimeStep per call. Multi-step iteration,
    retries, and reflection all live in AgentRuntime's control loop now —
    this class has no knowledge of the plan as a whole."""

    async def execute_step(
        self, context: RuntimeContext, step: RuntimeStep
    ) -> tuple[ToolResult, list[RuntimeEvent]]:
        events = [RuntimeEvent(type=STEP_STARTED, step=step.name)]
        ...
        try:
            ...
            logger.info("%s -> NOT FOUND", step.name)
            raise StepExecutionError(step.name, str(exc), events=events) from exc

        step.output = result.data if isinstance(result.data, dict) else {"data": result.data}
        ...
        events.append(
            RuntimeEvent(
                type=event_type,
                step=step.name,
                data={"duration_ms": result.duration_ms, "retries": result.retries},
            )
        )
        logger.info(
            "%s -> %s (%.1fms, retries=%d)",
    ...
    def _update_memory(self, context: RuntimeContext, tool_result: ToolResult) -> None:
        context.memory_manager.save_tool_result(context.state, tool_result)